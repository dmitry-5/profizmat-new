                ...
                return this.trigger("unmask").each((function() {
                    ...
                    function androidInputEvent(e) {
                        ...
                        if (oldVal && oldVal.length && oldVal.length > curVal.length) {
                            ...
                            if (pos.begin < len) if (!tests[pos.begin]) {
                                pos.begin++;
                                if (tests[pos.begin].test(lastEnteredValue)) pos.begin++;
                            } else if (tests[pos.begin].test(lastEnteredValue)) pos.begin++;
                            input.caret(pos.begin, pos.begin);
                        }
                        tryFireCompleted();
                    }
                    ...
                    function checkVal(allow) {
                        ...
                        if (allow) writeBuffer(); else if (lastMatch + 1 < partialPosition) if (settings.autoclear || buffer.join("") === defaultBuffer) {
                            if (input.val()) input.val("");
                            clearBuffer(0, len);
                        } else writeBuffer(); else {
                            writeBuffer();
